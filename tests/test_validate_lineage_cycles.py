...
class ValidateLineageCyclesTests(unittest.TestCase):
    ...
    def test_reports_malformed_json_with_line(self) -> None:
        path = self.root / "data/reports/reports.json"
        path.parent.mkdir(parents=True)
        path.write_text('[{"report_id":"rpt_a"},\ninvalid]', encoding="utf-8")
        errors = validate_repository(self.root)
        self.assertEqual(1, len(errors))
        self.assertIn("reports.json:2", errors[0])

    def test_rejects_invalid_relation_value_shape(self) -> None:
        self.write_ndjson(
            "data/source-items/items.ndjson",
            [{"source_item_id": "item_a", "upstream_item_ids": ["item_b", 7]}],
        )
        errors = validate_repository(self.root)
        self.assertEqual(1, len(errors))
        self.assertIn("upstream_item_ids must be a string, null, or an array of strings", errors[0])
